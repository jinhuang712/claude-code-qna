...
状态：v1 已定稿并实装，无待审项。最后一次与实装对账 2026-07-27 12:20:56（北京时间，下同）。

实装期间发现三处设计漏洞，均已回填：`.qna/` 落盘位置由模型计算（见第九节「一切走脚本」）、注入命令行里的 `|` 会被当成管道、注入文本里的 `$QNA_*` 在模型的 shell 里是空的（见第五节）。三者同一家族——**凡让模型自己算的落点，算错都不报错**。

首次真实试跑又暴露三条，也已回填：hook 把 `.qna/` 撒进每个开过会话的项目（见第五节「存储」）、总览与收尾用等宽账本读起来像日志转储（见第八节）、中途安装时把 hook / marker / 注入这些内部名词端给了用户（见 `ask.md` 第 0 步）。**外加一条没法靠改文档解决的**：扫描侧两次把「每次调用时重选的东西」做成了题，全靠用户驳回拦下——负面清单已补一条，但那一侧至今没有脚本兜底。

代码侧的机械保证由 `tests/smoke.sh` 逐条断言（50 条）；prompt 侧（R3 / R5 / R6 / R7、五类来源的召回、对账判断）无法脚本化，只能在真实会话里验。
...
| G2.2 | 题干自解释，不假设共享上下文 | `Q9: 根本不知道这是啥` | R3 |
...
| | 明确不做 | 为什么 |
|---|---|---|
| N1 | **不保证完整性** | 没有机制能强迫模型意识到自己在做决定。这是根本制约，不是待改进项 |
| N2 | 不自动执行决策 | 汇总完停下，不追问"要开工吗" |
| N3 | 不替代该当场问的问题 | 破坏性、不可逆、猜错就白干的仍然立刻问。停车不是不问的借口 |
| N4 | 不跨 session | 按 `session_id` 隔离，`/clear` 归零 |
...
| `Stop` 的 `additionalContext` 进入**下一次**模型调用 | 积压提醒实际显示在**下一轮回复末尾**，不是当轮结束瞬间。连续对话感觉不出；就此走开则最后一次不显示 |
...
```
  ...
  R2  选项下限 3 上限 4            禁是非题 · 有多少给多少 · 不凑数
  R3  选项自解释                   专有名词要说清它是什么
  R4  所有单选题必须带 preview     并排展示实物 · 默认 single-select
  R5  重头题解释压到一屏           细节下沉到 description/preview
      批次之间不写散文           总览是全命令唯一的叙述位置
...
| 归属 | 条目 | 依据 / 理由 |
|---|---|---|
| 已定 | 默认 TTL → `300` 秒 | 你 19:41 说「5 分钟就行」 |
| 已定 | 加 `--no-cache` | 你 19:38 直接让我加了 |
| 失效 | 缓存文件命名格式 | 后端定成 SQLite，没有文件名这回事了 |
| 默认处理 | 命中日志走 debug 级 | 琐碎，不占题，有异议叫停 |
| 要问 · 重头 | 缓存键要不要纳入认证身份 | — |
| 要问 · 重头 | 写入失败静默降级还是出声 | — |
| 要问 · 重头 | 并发写怎么处理 | — |
| 要问 · 顺手 | 过期清理的触发时机 | — |
| 要问 · 顺手 | 缓存统计怎么暴露 | — |
| 要问 · 顺手 | 要不要支持自定义缓存目录 | 你说过先记着 |
| 要问 · 顺手 | 缓存命中率要不要进 metrics | — |

判错了直接说，我把它拽回提问队列。

**一表到底、一行一条目，不用等宽账本。** 等宽块在窄屏折行就崩，而且读起来像日志转储；表格能直接把「依据」摆成一列，对账证据不再挤在括号里。收尾清单同一形态（见下）。
...
| 归属 | 条目 | 结果 / 理由 |
|---|---|---|
| 已定 | 缓存后端 | `SQLite` —— 连带：需处理并发写，下一步开 WAL |
| 已定 | 缓存键 | 纳入 token 指纹 —— 连带：缓存目录按 token 分裂 |
| 已定 | 默认 TTL | `300` 秒 |
| 已定 | 写入失败 | 打 warning 后降级 |
| 已定 | 并发写 | 开 WAL |
| 已定 | 过期清理 | 每次读时顺带清 |
| 已定 | 缓存统计 | `arkcli cache stats`，按 token 分开 |
| 默认处理 | 命中日志 | debug 级 |
| 失效 | 缓存文件命名格式 | 后端改 SQLite |
| 失效 | 自定义缓存目录 | 缓存键纳入身份后不成立 |
