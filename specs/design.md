...
| `preview` 只对 single-select 生效 | 默认一律 single-select；multiSelect 是带补偿义务的例外（见 §三之二）|
...
**怎么拿到 transcript 路径**：不能自己拼（`~/.claude/projects/` + cwd 转义是内部规则，且中途 `cd` 就错）。`SessionStart` hook 的输入 JSON 里直接给了 `transcript_path`，写进 `.qna/<session_id>.meta`，`qna-transcript` 从那里读。
...
`$CLAUDE_PROJECT_DIR/.qna/<session_id>.md`，append-only，按项目 + session 双重隔离。
...
已经定了的 · 2 条
  V  默认 TTL              -> 300 秒（你 19:41 说「5 分钟就行」）
  V  要不要 --no-cache     -> 加（你 19:38 直接让我加了）

失效 · 1 条
  X  缓存文件命名格式 —— 后端定成 SQLite，没有文件名这回事了

我按默认处理了 · 1 条   琐碎，不占题，有异议叫停
  ·  命中日志走 debug 级
...
```
已定
  缓存后端    SQLite            连带: 需处理并发写，下一步得开 WAL
  缓存键      纳入 token 指纹   连带: 缓存目录按 token 分裂
  默认 TTL    300 秒
  写入失败    打 warning 后降级
  并发写      开 WAL
  过期清理    每次读时顺带清
  缓存统计    arkcli cache stats，按 token 分开
