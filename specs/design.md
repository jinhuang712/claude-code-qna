...
状态：v1 已定稿并实装，无待审项。最后一次与实装对账 2026-07-27 14:55:17（北京时间，下同）。

实装期间发现三处设计漏洞，均已回填：`.qna/` 落盘位置由模型计算（见第九节「一切走脚本」）、注入命令行里的 `|` 会被当成管道、注入文本里的 `$QNA_*` 在模型的 shell 里是空的（见第五节）。三者同一家族——**凡让模型自己算的落点，算错都不报错**。

首次真实试跑又暴露三条，也已回填：hook 把 `.qna/` 撒进每个开过会话的项目（见第五节「存储」）、总览与收尾的排版（先是等宽账本像日志转储，换成表格后又把四种归属混装在一张表里，逼出一个笼统的「归属」列，现已一栏一表，见第八节）、中途安装时把 hook / marker / 注入这些内部名词端给了用户（见 `ask.md` 第 0 步）。**外加一条没法靠改文档解决的**：扫描侧两次把「每次调用时重选的东西」做成了题，全靠用户驳回拦下——负面清单已补一条，但那一侧至今没有脚本兜底。
...
| 条目 | 落到哪 | 你的原话 |
|---|---|---|
| 默认 TTL | `300` 秒 | 19:41「5 分钟就行」 |
| `--no-cache` | 已加 | 19:38「把这个 flag 加上」 |

**失效**

| 条目 | 为什么 |
|---|---|
| 缓存文件命名格式 | 后端定成 SQLite，没有文件名这回事了 |
...
**一栏一表，不用等宽账本。** 等宽块在窄屏折行就崩，读起来像日志转储；表格能把「依据」摆成一列，对账证据不再挤在括号里。而**已定和还要问不能共用一张表**——挤在一起就得设个笼统的「归属」列，四种归属被迫共享同一套列名，「你的原话」「我怎么做的」「分级」这些各自真正需要的信息只能压成一句「理由」。拆开之后每张表只带自己那几列，扫一眼就知道哪些不用管、哪些等你拍。收尾清单同一形态（见下）。
...
| 条目 | 结果 | 连带 |
|---|---|---|
| 缓存后端 | `SQLite` | 需处理并发写，下一步开 WAL |
| 缓存键 | 纳入 token 指纹 | 缓存目录按 token 分裂 |
| 默认 TTL | `300` 秒 | — |
| 写入失败 | 打 warning 后降级 | — |
| 并发写 | 开 WAL | — |
| 过期清理 | 每次读时顺带清 | — |
| 缓存统计 | `arkcli cache stats` | 按 token 分开统计 |
