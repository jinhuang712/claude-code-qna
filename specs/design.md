...
代码侧的机械保证由 `tests/smoke.sh` 逐条断言（92 条）：选项下限与上限、preview 必填、`--where` 必须落到真实 file:line 或原话、归档必须带原话、四条命令的路径全注入、marker 的生效范围与自然过期、目录懒建。prompt 侧（R3 / R5 / R6 / R7、五类来源的召回、对账判断）无法脚本化，只能在真实会话里验。
...
| | 明确不做 | 为什么 |
|---|---|---|
| N1 | **不保证完整性** | 没有机制能强迫模型意识到自己在做决定。这是根本制约，不是待改进项。实测代价：某 54 分钟会话 84 轮对话、49 次提问，`qna-add` 零调用——协议只在第 0 轮注入过一次，之后无任何复发触发点。故 `Stop` hook 在**零条目**会话里按 15 / 45 / 90 轮各提醒一次（见第六节）|
...
### `Stop` 的 additionalContext 是给用户看的

**实测纠正过一次错误前提。** 提醒最初写成一段 150 词的指令块，结尾还写着「别把这个检查告诉用户」——因为我以为它只进模型上下文。Claude Code 把 `Stop` hook 的输出渲染成 **"Stop hook feedback"** 显示在对话流里，用户把整段都看到了，包括一条裸的绝对路径，反馈是「这段很奇怪」。

**先试着让它别显示。** hook 协议里有 `suppressOutput`，两条输出都带上了它。但**能不能对这条路径生效，从这里无法证明**——用户看到的是被渲染成 "Stop hook feedback" 的那一路，未必是 `suppressOutput` 管的那一路。所以文字本身也按「会被看到」写：提醒压到 254 字符一句话（附命令行的那版更长，长在路径上，不是长在措辞上）。**两手都做，因为其中一手无法验证。**

两条由此确立的纪律：

| 纪律 | 为什么 |
|---|---|
| 每一行都按「人也会读」来写 | 短句、平白话、不用指令口吻，能陈述就不要命令 |
| **不要让模型把它复述一遍** | hook 的输出本身就是呈现。让模型再说一次等于同一条消息出现两遍——用户对计数提示的原话是「这个看起来非常突兀」 |

**计数提示要自解释。** 原文是 `qna: 1 parked — /qna:ask to settle`，用户的原话是「不知道这个是啥意思」——它既没说记的是什么，也没说 `/qna:ask` 会干什么。现在带上条目标题，并说清 `/qna:ask` 把它变成一道可点选的问题。
...
**修法：`Stop` 在零条目会话里按 15 / 45 / 90 轮各提醒一次。** 三个设计约束：

| 约束 | 怎么满足 |
|---|---|
| 不能写盘 | 提醒次数**从 transcript 里数**——`additionalContext` 本身会落成 `hook_additional_context` 条目，已经是磁盘上的记录，不需要第二份。这条是硬要求：`Stop` 无处可写而不在每个开过会话的项目里留下 `.qna/` |
| 不能把判断权搬走 | 提醒只把协议重新拉回视野、把命令行摊开。**「模型干活中途主动打断自己、自我判定」是设计本身，不是要绕开的东西** |
| 不能变成注水机 | 提醒里明写「一条都不过关也是个真答案，别说话，往下走」 |

**当年砍掉的是每轮提醒**，理由写在注释里：「回顾式自检很弱，每轮一次的成本换来的东西很少」。现在量出了对照组：换来的是 0 条。里程碑提醒是不同的成本档——一个会话最多 3 次。
